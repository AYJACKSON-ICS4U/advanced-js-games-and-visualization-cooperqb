/*
The sticks yellow sticks will increase your score, 
the other ones will reduce your score. 
If you get to a score of 35, you win!!!
*/
...
Sparky.prototype.draw = function() {
    fill(255, 0, 26);
    this.y = constrain(this.y, 0, height-40 );
    image(this.img, this.x, this.y, 40, 40);
};

Sparky.prototype.hop = function() {
    this.img = getImage("avatars/starky-sapling");
    this.y -= 5;
};
 
Sparky.prototype.fall = function() {
    this.img = getImage("avatars/starky-sapling" );
    this.y += 5;
};

Sparky.prototype.checkForStickGrab = function(stick) {
...
draw = function() {
    // static
    background(47, 124, 143);
    fill(179, 52, 105);
    rectMode(CORNER);
    rect(-2, height*0.89, width, height*0.12);
    
    for (var i = 0; i < grassXs.length; i++) {
        image(getImage("cute/Selector"), grassXs[i], height*0.85, 20, 20);
        grassXs[i] -= 1;
        if (grassXs[i] <= -20) {
            grassXs[i] = width;
        }
    ...
    for (var i = 0; i < pylon.length; i++) {
        pylon[i].draw();
        sparky.checkForStickGrab(pylon[i]);   
        pylon[i].x -= 1;
        if(pylon[pylon.length-1].x < 0 && score >=35){ //If the last stick is past the screen and your score is at least 35, you win!
            ...
            fill(56, 54, 54);
            textSize(24);
            text("Congradulations!!!",80,150,200,200);
            textSize(14);
            fill(255, 255, 255);
            text("Good Job",80,180,200,200);
